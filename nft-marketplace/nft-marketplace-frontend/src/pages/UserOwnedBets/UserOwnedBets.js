...
function UserOwnedBets({ user }) {
  const [certificates, setCertificates] = useState([]);

  useEffect(() => {
    const fetchCertificates = async () => {
      try {
        const response = await fetch(`http://localhost:8100/api/allCertificatesOwned/${user.address}`);
        const data = await response.json();
        
        // Filtra i certificati con bettertesta o bettercroce uguale a user.address
        const userCertificates = data.filter(cert =>
          cert.owner === user.address
        );
